...
const SUBJECTS = ["Course Enquiry", "Internship Application", "Hiring Partnership", "Trainer / Mentor Role", "Research Collaboration", "Other"];
          ...
          <div className="lg:col-span-3 rounded-2xl border border-white/8 bg-white/[0.02] p-8">
            {sent ? (
              <div className="flex flex-col items-center justify-center h-full py-12 text-center">
                ...
                <p className="text-gray-400 text-sm mb-1">A copy has been forwarded to info@nexgenbrtechnologies.com</p>
              ...
              <form onSubmit={onSubmit} className="space-y-5">
                ...
                <div className="grid sm:grid-cols-2 gap-5">
                  <div><label className={labelCls}>Full Name *</label><input className={inputCls} placeholder="Rahul Sharma" value={form.name} onChange={onChange("name")} required /></div>
                  <div><label className={labelCls}>Email Address *</label><input type="email" className={inputCls} placeholder="rahul@email.com" value={form.email} onChange={onChange("email")} required /></div>
                </div>
                <div className="grid sm:grid-cols-2 gap-5">
                  <div><label className={labelCls}>Phone Number *</label><input className={inputCls} placeholder="+91 91234 56789" value={form.phone} onChange={onChange("phone")} /></div>
                  <div>
                ...
                <div>
                  ...
                  <textarea className={`${inputCls} h-32 pt-3 resize-none`} placeholder="Tell us which course you're interested in, batch dates, or anything else…" value={form.message} onChange={onChange("message")} required />
                </div>
                <div><label className={labelCls}>Resume / Portfolio Link (optional)</label><input className={inputCls} placeholder="https://github.com/yourprofile or drive.google.com/..." value={form.resume} onChange={onChange("resume")} /></div>
                <button type="submit" disabled={loading} className="w-full py-3 rounded-xl text-white font-bold text-sm transition-all hover:opacity-90 active:scale-95 disabled:opacity-60" style={{ background: "linear-gradient(135deg,#4f6ef7 0%,#7c3aed 100%)" }}>
            ...
            {[
              ...
              <div key={item.title} className="rounded-2xl border border-white/8 bg-white/[0.02] p-5 flex gap-4">
                ...
                <div>
                  ...
                  {item.link ? <a href={item.link} className="text-gray-300 text-sm hover:text-cyan-400 transition-colors">{item.detail}</a> : <p className="text-gray-300 text-sm">{item.detail}</p>}
